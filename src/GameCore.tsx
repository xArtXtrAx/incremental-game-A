...
const particleDirections = [
  { x: 0, y: -82 }, { x: 58, y: -58 }, { x: 82, y: 0 },
  { x: 58, y: 58 }, { x: 0, y: 82 }, { x: -58, y: 58 },
  { x: -82, y: 0 }, { x: -58, y: -58 },
]
      ...
      {Array.from({ length: count }, (_, index) => {
        ...
        return (
          <motion.span
            ...
            initial={false}
            animate={{
              opacity: active ? [0.42, 1, 0.42] : isCharged ? 1 : 0.14,
              scaleY: active ? [0.8, 1.35, 0.8] : 1,
            }}
            transition={active
              ? { duration: 0.85, delay: (index % 12) * 0.035, repeat: Infinity }
              : { duration: 0.14 }}
            style={{
              position: 'absolute', top: '50%', left: '50%',
              width: active ? 2 : 3, height: active ? 8 : 11,
              marginTop: active ? -4 : -5.5, marginLeft: active ? -1 : -1.5,
              borderRadius: 999,
              background: active ? '#d5fbff' : isCharged ? '#8ce8ff' : 'rgba(70, 145, 195, 0.42)',
              boxShadow: active || isCharged ? '0 0 8px rgba(42, 205, 255, 0.95)' : 'none',
...
export function GameCore({
  ...
  const clickPower = getClickPower(game.clickLevel, game.manualClicks, game.pressureLevel, overloadMultiplier)
  const production = getEnergyPerSecond(
    game.generatorLevel, game.resonanceLevel, game.manualClicks,
    game.pressureLevel, overloadMultiplier,
  )
  const cavitationThreshold = getCavitationClicksRequired(game.cavitationLevel)
  const cavitationReward = getCavitationReward(
    game.generatorLevel, game.resonanceLevel, game.manualClicks,
    game.pressureLevel, game.cavitationLevel, overloadMultiplier,
  )
  const pressureOpacity = game.pressureLevel > 0
    ...
    <section className="core-column" aria-label="Núcleo de energía">
      ...
      <div className="button-stage">
        ...
        <motion.button
          ...
          transition={overloadActive ? { duration: 0.75, repeat: Infinity } : undefined}
          aria-label={`Generar ${clickPower} de energía. Sobrecarga ${game.overloadCharge} de ${overloadThreshold}.`}
        >
          <motion.span
            className="sphere-liquid"
            aria-hidden="true"
            animate={cavitationBurst
              ? { x: [0, -4, 4, -3, 3, 0], rotate: [0, -1.2, 1.2, 0] }
              : overloadActive ? { x: [0, -1.5, 1.5, 0], y: [0, -1, 1, 0] } : { x: 0, y: 0 }}
            transition={cavitationBurst
              ? { duration: 0.68 }
              : overloadActive ? { duration: 0.32, repeat: Infinity } : { duration: 0.2 }}
          >
          ...
          <span aria-hidden="true" style={{ position: 'absolute', inset: 0, zIndex: 4, opacity: pressureOpacity, pointerEvents: 'none' }}>
            {[0, 1, 2].map((ring) => (
              <motion.span
                key={ring}
                initial={{ scale: 0.38, opacity: 0 }}
                animate={{ scale: [0.38, 0.72, 1.08], opacity: [0, 0.72, 0] }}
                transition={{ duration: Math.max(2.8, 6 - game.pressureLevel * 0.4), delay: ring * 0.75, repeat: Infinity }}
                style={{
                  position: 'absolute', top: '50%', left: '50%', width: 92, height: 42,
                  marginTop: -21, marginLeft: -46, border: '1px solid rgba(83,207,255,.72)',
                  borderRadius: '50%', boxShadow: '0 0 12px rgba(0,155,255,.35)',
                }}
              />
            ))}
          </span>
          ...
          {overloadActive && (
            <motion.span
              ...
              style={{
                position: 'absolute', inset: 5, zIndex: 4, borderRadius: '50%',
                border: '2px solid rgba(195,249,255,.92)',
                background: 'radial-gradient(circle,rgba(81,205,255,.42),rgba(0,84,205,.08) 58%,transparent 74%)',
                boxShadow: '0 0 28px rgba(73,220,255,.92),inset 0 0 28px rgba(83,210,255,.5)',
              }}
            />
          )}
          {cavitationBurst && (
            <motion.span
              ...
              animate={{ opacity: [0.9, 0.58, 0], scale: [0.18, 0.82, 1.45] }}
              transition={{ duration: 0.82 }}
              style={{ position: 'absolute', inset: 7, zIndex: 5, border: '2px solid #a4f1ff', borderRadius: '50%', boxShadow: '0 0 32px #3fcdff' }}
            />
          )}
          <span className="sphere-shine" aria-hidden="true" />
          <span className="button-label">
            <strong>{overloadActive ? 'OVERLOAD' : 'CLICK'}</strong>
            <small>{overloadActive
              ? `${overloadRemaining.toFixed(1)} s · ×${format.format(overloadMultiplier)}`
              : `${format.format(sphereClicks)} / ${format.format(SPHERE_CLICK_CAPACITY)}`}</small>
          </span>
        </motion.button>

        {bursts.map((burst) => (
          <div className="click-burst" key={burst.id} aria-hidden="true">
            <motion.span className="click-plus-one" initial={{ opacity: 0, scale: 0.65, y: 0 }} animate={{ opacity: [0, 1, 1, 0], scale: [0.65, 1.08, 1], y: -92 }} transition={{ duration: 0.8 }}>
              +{format.format(burst.amount)}
            </motion.span>
            {particleDirections.map((direction, index) => (
              <motion.span className="click-particle" key={`${burst.id}-${index}`} initial={{ opacity: 0, scale: 0.25, x: 0, y: 0 }} animate={{ opacity: [0, 1, 0], scale: [0.25, 1.15, 0.2], x: direction.x, y: direction.y }} transition={{ duration: 0.65 }} />
            ))}
          </div>
        ))}

        {cavitationBurst && (
          <motion.div key={cavitationBurst.id} role="status" initial={{ opacity: 0, y: 12 }} animate={{ opacity: [0, 1, 1, 0], y: [12, -10, -24] }} transition={{ duration: 1.35 }} style={{ position: 'absolute', top: 6, zIndex: 8, color: '#dffbff', fontWeight: 900, textShadow: '0 0 18px #00aeff' }}>
            DESCARGA +{format.format(cavitationBurst.amount)}
          </motion.div>
        )}
        {overloadBurst && (
          <motion.div key={overloadBurst.id} role="status" initial={{ opacity: 0, scale: 0.55, y: 28 }} animate={{ opacity: [0, 1, 1, 0], scale: [0.55, 1.12, 1], y: [28, -8, -34] }} transition={{ duration: 1.7 }} style={{ position: 'absolute', top: -8, zIndex: 9, color: '#effeff', fontWeight: 950, textShadow: '0 0 25px #17ccff' }}>
      ...
      {game.cavitationLevel > 0 && (
        <div className="sphere-status" style={{ marginTop: -6 }}>
          ...
          <strong>{game.cavitationCharge}/{cavitationThreshold} · Próxima +{format.format(cavitationReward)}</strong>
        </div>
      )}
      {game.overloadLevel > 0 && (
        <div className={`sphere-status${overloadActive ? ' is-full' : ''}`} style={{ marginTop: -6 }}>
          <span>Sobrecarga</span>
          <strong>{overloadActive
            ? `ACTIVA ×${format.format(getOverloadMultiplier(game.overloadLevel))} · ${overloadRemaining.toFixed(1)} s`
            : `${game.overloadCharge}/${overloadThreshold} · ×${format.format(getOverloadMultiplier(game.overloadLevel))} por ${overloadDuration} s`}</strong>
        </div>
      )}
