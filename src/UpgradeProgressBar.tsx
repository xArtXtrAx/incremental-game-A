...
function getTransitionDuration(item: UpgradeProgressDefinition) {
  if (item.id === 'overload-active' || item.id === 'refraction-active') {
    return 140
  }
...
function ProgressBar({ item }: { item: UpgradeProgressDefinition }) {
  ...
  const style: ProgressStyle = {
    '--upgrade-progress': progress.toFixed(6),
    '--upgrade-segment-size': `${(100 / segments).toFixed(5)}%`,
    '--upgrade-transition-ms': `${getTransitionDuration(item)}ms`,
  }
